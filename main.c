...
int LerMapa(char *NomedoMapa, char MapadoJogo[LINHAS][COLUNAS], int Linhas, int Colunas, int maxEnemies, int *qtdEnemies, ENEMY *enemies, PLAYER *Jogador){
    int I,J;
    FILE *Arquivo;
    int Status = 0;
    *qtdEnemies = 0;
...
int colision(char MapadoJogo[LINHAS][COLUNAS], PLAYER *Jogador){
    ...
    switch(MapadoJogo[Jogador -> PosY][Jogador -> PosX]){
        ...
        case(OBSTACULO): col = 1; //se a posição a ser ocupada pelo jogador é um obstáculo ele retorna um código 1 para a função de movimentação
                               break;

        case(TESOURO): col = 2; //se a posição a ser ocupada pelo jogador é o tesouro ele retorna um código 2 para a função de movimentação
                           break;

        case(ZUMBI): col = 3; //se a posição a ser ocupada pelo jogador é um zombie ele retorna um código 3 para a função de movimentação
                             break;

        case(TROLL): col = 4; //se a posição a ser ocupada pelo jogador é um troll ele retorna um código 4 para a função de movimentação
                            break;
        default: break;
    }
...
void move(char MapadoJogo[LINHAS][COLUNAS], PLAYER *Jogador, int x, int y, int *flag){
    ...
    switch(colision(MapadoJogo, Jogador)){
...
void move_enemies(char MapadoJogo[LINHAS][COLUNAS], ENEMY enemies[MAXIMODEMONSTROS1], int qnt, PLAYER *Jogador, int *flag){

    int i, x = 0, y = 0;
    char symbol;
    printf("\nVidas: %d", Jogador->Vidas+1);
    ...
    for(i = 0; i < qnt; i++)
    {
        ...
        symbol = MapadoJogo[enemies[i].pos_y + y][enemies[i].pos_x + x];
        if((symbol == OBSTACULO && enemies[i].enemyType == ENEMY_TYPE_TROLL) || (enemies[i].steps > 4) || symbol == PAREDE){
            while(enemies[i].direction == rand()%4){
                enemies[i].direction = rand()%4;
            }
            enemies[i].steps = 0;
        }
        else if(enemies[i].enemyType == ENEMY_TYPE_ZOMBIE && MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == OBSTACULO){

            MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = OBSTACULO;
            (enemies[i].pos_y) = (enemies[i].pos_y) + y;
            (enemies[i].pos_x) = (enemies[i].pos_x) + x;
            enemies[i].steps = enemies[i].steps + 1;
        }
        else if((enemies[i].enemyType == ENEMY_TYPE_ZOMBIE && MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == TESOURO) || (enemies[i].enemyType == ENEMY_TYPE_TROLL && MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == TESOURO)){
            MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = TESOURO;
            (enemies[i].pos_y) = (enemies[i].pos_y) + y;
            (enemies[i].pos_x) = (enemies[i].pos_x) + x;
            enemies[i].steps = enemies[i].steps + 1;

        }
        else if(MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == MapadoJogo[Jogador -> PosY][Jogador -> PosX]){
            *flag = 1;
            switch(enemies[i].enemyType){
                case ENEMY_TYPE_TROLL: MapadoJogo[Jogador -> PosY][Jogador -> PosX] = TROLL;
                                       break;
                case ENEMY_TYPE_ZOMBIE: MapadoJogo[Jogador -> PosY][Jogador -> PosX] = ZUMBI;
                                        break;
            }

        }
        else{
            MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = VAZIO;
            (enemies[i].pos_y) = (enemies[i].pos_y) + y;
            (enemies[i].pos_x) = (enemies[i].pos_x) + x;
            enemies[i].steps = enemies[i].steps + 1;
        }
        ...
        switch(enemies[i].enemyType){
            case ENEMY_TYPE_TROLL: if(MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == TESOURO){
                                        MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = TESOURO;
                                    }
                                   MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = TROLL;
                                   break;
            case ENEMY_TYPE_ZOMBIE: if(MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] == TESOURO){
                                        MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = TESOURO;
                                    }
                                    else if(MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] != OBSTACULO)
                                        MapadoJogo[enemies[i].pos_y][enemies[i].pos_x] = ZUMBI;
                                    break;
        }
    }
}
...
int main()
    ...
    while(Jogador.Vidas >0)  // laco da execucao, cai aqui apos 1 TAB e continua ate Pressionado sair
    {
